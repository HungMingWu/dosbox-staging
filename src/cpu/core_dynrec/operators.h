...
static Bit8u DRC_CALL_CONV dynrec_rol_byte(Bit8u op1,Bit8u op2) {
	if (!(op2&0x7)) {
		if (op2&0x18) {
			FillFlagsNoCFOF();
			SETFLAGBIT(FLAG_CF, op1 & 1);
			SETFLAGBIT(FLAG_OF, (op1 & 1) ^ (op1 >> 7));
		}
		return op1;
	}
	FillFlagsNoCFOF();
...
static Bit8u DRC_CALL_CONV dynrec_ror_byte(Bit8u op1,Bit8u op2) {
	if (!(op2&0x7)) {
		if (op2&0x18) {
			FillFlagsNoCFOF();
			SETFLAGBIT(FLAG_CF, op1 >> 7);
			SETFLAGBIT(FLAG_OF, (op1 >> 7) ^ ((op1 >> 6) & 1));
		}
		return op1;
	}
	FillFlagsNoCFOF();
...
static Bit16u DRC_CALL_CONV dynrec_rol_word(Bit16u op1,Bit8u op2) {
	if (!(op2&0xf)) {
		if (op2&0x10) {
			FillFlagsNoCFOF();
			SETFLAGBIT(FLAG_CF, op1 & 1);
			SETFLAGBIT(FLAG_OF, (op1 & 1) ^ (op1 >> 15));
		}
		return op1;
	}
	FillFlagsNoCFOF();
...
static Bit16u DRC_CALL_CONV dynrec_ror_word(Bit16u op1,Bit8u op2) {
	if (!(op2&0xf)) {
		if (op2&0x10) {
			FillFlagsNoCFOF();
			SETFLAGBIT(FLAG_CF, op1 >> 15);
			SETFLAGBIT(FLAG_OF, (op1 >> 15) ^ ((op1 >> 14) & 1));
		}
		return op1;
	}
	FillFlagsNoCFOF();
